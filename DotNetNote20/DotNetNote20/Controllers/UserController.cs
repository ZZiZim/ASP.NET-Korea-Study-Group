            ...
            if (ModelState.IsValid)
            {
                if (_repository.IsCorrectUser(model.UserId, model.Password))
                {
                    ...
                    var claims = new List<Claim>()
                  {
                    ...
                    // await HttpContext.Authentication.SignInAsync("Cookies", new ClaimsPrincipal(ci)); -- Core 1.x

                    await HttpContext.SignInAsync("Cookies", new ClaimsPrincipal(ci)); // Core 2.x
